...
void modbus_slave_thread(void *param)
{
    ...
    modbus_rtu_init(&modbus_slave,NULL,&modbus_slave_config);
    modbus_rtu_config(&modbus_slave,"/dev/uart3", 9600, 8, 1, 'N');
    modbus_rtu_set_rts_ops(&modbus_slave,_modbus_slave_rts_set);
    ...
    while (1)
    {
			...
			{
			    MODBUS_PRINTF("\n[1]------ receive %d --------\n",rc);
				//rc = modbus_reply(&modbus_slave, receive_buf, rc, modbus_mapping);
...
void modbus_master_thread(void *param)
{
    ...
    modbus_rtu_init(&modbus_master,NULL,&modbus_master_config);
    modbus_rtu_config(&modbus_master,"/dev/uart6", 9600, 8, 1, 'N');
    modbus_rtu_set_rts_ops(&modbus_master,_modbus_master_rts_set);
